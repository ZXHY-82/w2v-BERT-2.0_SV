...
class Wav2Vec2BertFeatureProjection(nn.Module):
    ...
    def get_num_params(self):
        return self.projection.in_features * 2 + self.projection.in_features * self.projection.out_features + self.projection.out_features

# change
# 新增init参数：layer_idx, ffn_idx(每层有两个ffn模块)
# 对intermediate_size进行裁剪, hidden_size不变
# config 新增参数: intermediate_size_group, use_feed_forward, limit_l, limit_r, temperature
class Wav2Vec2BertFeedForward(nn.Module):
    def __init__(self, config, layer_idx, ffn_idx, act_fn=None, hidden_size=None):
        super().__init__()
        ...
        self.intermediate_dense = nn.Linear(hidden_size, config.intermediate_size_group[layer_idx][ffn_idx])
        self.intermediate_act_fn = ACT2FN[act_fn] if isinstance(act_fn, str) else act_fn

        self.output_dense = nn.Linear(config.intermediate_size_group[layer_idx][ffn_idx], hidden_size)
        self.output_dropout = nn.Dropout(config.hidden_dropout)

        if config.prune:
            self.hard_concrete_for_intermediate = HardConcrete(
                n_in=config.intermediate_size_group[layer_idx][ffn_idx], init_mean=0.5,
                limit_l=config.limit_l, limit_r=config.limit_r, temperature=config.temperature
            )
        else:
    ...
    def forward(self, hidden_states):
        ...
        if self.hard_concrete_for_intermediate:
            intermediate_mask = self.hard_concrete_for_intermediate()  # (intermediate_size,)
            hidden_states = hidden_states * intermediate_mask
    ...
    def prune(self):
        new_config = {
            ...
            "intermediate_size_group": self.intermediate_dense.out_features
        }
        if self.hard_concrete_for_intermediate is not None:
            ...
            new_config["intermediate_size_group"] = len(interm_index)
            if new_config["intermediate_size_group"] == 0:
...
# change
# 新增init参数：layer_idx
# config 新增参数：conv_group[[1,2,3] * layer_num]
class Wav2Vec2BertConvolutionModule(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.layer_norm = nn.LayerNorm(config.conv_group[layer_idx][0], eps=config.layer_norm_eps)
        self.pointwise_conv1 = nn.Conv1d(
            config.conv_group[layer_idx][0],
            2 * config.conv_group[layer_idx][1],
            kernel_size=1,
            stride=1,
            padding=0,
            bias=False,
        ...
        self.depthwise_conv = nn.Conv1d(
            config.conv_group[layer_idx][1],
            config.conv_group[layer_idx][1],
            config.conv_depthwise_kernel_size,
            stride=1,
            padding=0,
            groups=config.conv_group[layer_idx][1],
            bias=False,
        )

        self.depthwise_layer_norm = nn.LayerNorm(config.conv_group[layer_idx][1], eps=config.layer_norm_eps)
        ...
        if config.prune:
            self.hard_concrete_pointwise_conv1 = HardConcrete(
                n_in=config.hidden_size, init_mean=0.01,
                limit_l=config.limit_l, limit_r=config.limit_r, temperature=config.temperature
            )
            # self.hard_concrete_depthwise_conv = HardConcrete(
            #     n_in=config.hidden_size, init_mean=0.01,
            #     limit_l=config.limit_l, limit_r=config.limit_r, temperature=config.temperature
            # )
        else:
            self.hard_concrete_pointwise_conv1 = None
            # self.hard_concrete_depthwise_conv = None
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        if self.hard_concrete_pointwise_conv1 is not None:
            channel_mask = self.hard_concrete_pointwise_conv1() # (hidden_size,)
            channel_mask = channel_mask.repeat(2).unsqueeze(-1)
            hidden_states = hidden_states * channel_mask
        ...
        if self.hard_concrete_pointwise_conv1 is not None:
            channel_mask = self.hard_concrete_pointwise_conv1()
            hidden_states = hidden_states * channel_mask.unsqueeze(-1)
        ...
        hidden_states = self.dropout(hidden_states)
        # if self.hard_concrete_pointwise_conv2 is not None:
        #     channel_mask = self.hard_concrete_pointwise_conv2()
        #     hidden_states = hidden_states * channel_mask.unsqueeze(-1)
    ...
    def get_num_params(self):
        ...
        num_params = num_params + self.layer_norm.normalized_shape[0] * 2
        if self.hard_concrete_pointwise_conv1 is not None:
            out_channel = self.hard_concrete_pointwise_conv1.l0_norm()
        else:
            out_channel = self.pointwise_conv1.out_channels
        num_params = num_params + self.pointwise_conv1.in_channels * out_channel * 1
        # in_channel = out_channel//2
        if self.hard_concrete_pointwise_conv1 is not None:
            out_channel = self.hard_concrete_pointwise_conv1.l0_norm()
        else:
            out_channel = self.depthwise_conv.out_channels
        num_params = num_params + out_channel * 31 + out_channel * 2
        in_channel = out_channel
        num_params = num_params + in_channel * self.pointwise_conv2.out_channels * 1
        return num_params
    
    def prune(self):
        new_config = [self.pointwise_conv1.in_channels]
        if self.hard_concrete_pointwise_conv1 is not None:
            assert not self.hard_concrete_pointwise_conv1.training
            mask = self.hard_concrete_pointwise_conv1() # (out_channel, )
            index = mask.nonzero().squeeze(-1) # (out_channel)
            assert len(index) > 0, f'conv1 out channel prune to zero in layer{self.layer_idx}'
            new_config.append(len(index))
            index_conv1 = index.repeat(2)
            prune_conv1d_layer(self.pointwise_conv1, index_conv1, 'output')
            # group 卷积只能裁剪输出通道 [out, in/groups. kernelsize]
            # prune_conv1d_layer(self.depthwise_conv, index, 'input')
            
        if self.hard_concrete_pointwise_conv1 is not None:
            assert not self.hard_concrete_pointwise_conv1.training
            mask = self.hard_concrete_pointwise_conv1()
            index = mask.nonzero().squeeze(-1)
            assert len(index) > 0, f'conv_dep out channel prune to zero in layer{self.layer_idx}'
            prune_conv1d_layer(self.depthwise_conv, index, 'output')
            prune_layer_norm(self.depthwise_layer_norm, index)
            prune_conv1d_layer(self.pointwise_conv2, index, 'input')
        # new_config.append(self.pointwise_conv2.out_channels)
        return new_config
...
# change
# 新增init参数：layer_idx， 对head_num进行裁剪
# config 新增参数: num_attention_heads_group, use_attention
class Wav2Vec2BertSelfAttention(nn.Module):
    """Construct an Wav2Vec2BertSelfAttention object.
    Can be enhanced with rotary or relative position embeddings.
    """

    def __init__(self, config, layer_idx, is_adapter_attention=False):
        super().__init__()
        ...
        self.num_heads = config.num_attention_heads_group[layer_idx]
        self.position_embeddings_type = config.position_embeddings_type if not is_adapter_attention else None

        self.linear_q = nn.Linear(hidden_size, self.head_size*self.num_heads)
        self.linear_k = nn.Linear(hidden_size, self.head_size*self.num_heads)
        self.linear_v = nn.Linear(hidden_size, self.head_size*self.num_heads)
        self.linear_out = nn.Linear(self.head_size*self.num_heads, hidden_size)
        ...
        if config.prune:
            self.hard_concrete_for_heads = HardConcrete(
                n_in=self.num_heads, init_mean=0.01,
                limit_l=config.limit_l, limit_r=config.limit_r, temperature=config.temperature)
        else:
    ...
    def forward(
        ...
        if self.hard_concrete_for_heads is not None:
            head_mask = self.hard_concrete_for_heads() # *(head, )
            hidden_states = hidden_states * head_mask.unsqueeze(-1).unsqueeze(-1)
    ...
    def get_num_params(self):
        ...
        num_params = (self.hidden_size + 1) * num_heads * self.head_size * 3 \
            + (num_heads * self.head_size + 1) * self.hidden_size
        num_params = num_params + self.distance_embedding.num_embeddings * self.distance_embedding.embedding_dim
    ...
    def prune(self):
        ...
        if self.hard_concrete_for_heads is not None:
            ...
            new_config["num_attention_heads"] = len(head_mask.nonzero())
            if new_config["num_attention_heads"] == 0:
                new_config["use_attention"] = False
            else:
                full_mask = head_mask.repeat_interleave(self.head_size)
                full_index = full_mask.nonzero().squeeze(-1)  # 1D

                prune_linear_layer(self.linear_q, full_index, "output")
                prune_linear_layer(self.linear_k, full_index, "output")
                prune_linear_layer(self.linear_v, full_index, "output")

                self.linear_out.weight.data *= full_mask
                prune_linear_layer(self.linear_out, full_index, "input")
            self.hard_concrete_for_heads = None
...
class Wav2Vec2BertEncoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config, layer_idx):
        ...
        # Feed-forward 1
        if config.use_feed_forward[layer_idx][0]:
            self.ffn1_layer_norm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
            self.ffn1 = Wav2Vec2BertFeedForward(config, layer_idx, 0)
        else:
            self.ffn1 = None

        # Self-Attention
        if config.use_attention[layer_idx]:
            self.self_attn_layer_norm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
            self.self_attn_dropout = nn.Dropout(dropout)
            self.self_attn = Wav2Vec2BertSelfAttention(config, layer_idx)
        else:
            self.self_attn = None

        # Conformer Convolution
        self.conv_module = Wav2Vec2BertConvolutionModule(config, layer_idx)

        # Feed-forward 2
        if config.use_feed_forward[layer_idx][1]:
            self.ffn2_layer_norm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
            self.ffn2 = Wav2Vec2BertFeedForward(config, layer_idx, 1)
        else:
            self.ffn2 = None
    ...
    def get_num_params(self):
        num_params = self.embed_dim * 2 + self.conv_module.get_num_params()
        if self.ffn1 is not None:
            num_params = num_params + self.ffn1.get_num_params() + self.embed_dim * 2
        if self.self_attn is not None:
            num_params = num_params + self.self_attn.get_num_params() + self.embed_dim * 2
        if self.ffn2 is not None:
            num_params = num_params + self.ffn2.get_num_params() + self.embed_dim * 2
        return num_params
...
class Wav2Vec2BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([Wav2Vec2BertEncoderLayer(config, i) for i in range(config.num_hidden_layers)])
    ...
    def prune(self):
        ...
        for layer in self.layers:
            ffn1_config = layer.ffn1.prune()
            ffn2_config = layer.ffn2.prune()
            att_config = layer.self_attn.prune()
            conv_config = layer.conv_module.prune()
            config['intermediate_size_group'].append([ffn1_config['intermediate_size_group'], ffn2_config['intermediate_size_group']])
            config['use_feed_forward'].append([ffn1_config['use_feed_forward'], ffn2_config['use_feed_forward']])
            config['conv_group'].append(conv_config)
            config['num_attention_heads_group'].append(att_config['num_attention_heads'])
            config['use_attention'].append(att_config['use_attention'])
        return config
...
class Wav2Vec2BertModel(Wav2Vec2BertPreTrainedModel):
    ...
    def get_num_params(self):
        return self.feature_projection.get_num_params() + self.encoder.get_num_params()

    def prune(self):
        config = self.encoder.prune()
        return config
...
class Wav2Vec2BertDemo(nn.Module):
